...
class BindingApi extends BladeApi {
    ...
    onValueChange_(ev) {
        ...
        const changeEvt = new TpChangeEvent(this, forceCast(value.binding.target.read()), ev.options.last);
        // keep legacy behavior
        this.emitter_.emit('change', changeEvt);
        // only for user-driven changes
        if (ev.internal === true) {
            this.emitter_.emit('internal-change', changeEvt);
        }
    }
}
...
class InputBindingValue {
    constructor(value, binding) {
        ...
        // Track whether the upcoming change is from user interaction (GUI)
    ...
    onValueChange_(ev) {
        this.push();
        const forwarded = Object.assign(Object.assign({}, ev), {
            sender: this,
            internal: this.__lastEventInternal
        });
        this.emitter.emit('change', forwarded);
        // default back to "internal" so subsequent UI gestures are tagged correctly
        this.__lastEventInternal = true;
    }
}
...
class RackApi {
    ...
    onRackValueChange_(ev) {
        ...
        const changeEvt = new TpChangeEvent(
          api,
          binding ? binding.target.read() : bc.value.rawValue,
          ev.options.last
        );
// always keep legacy 'change'
        this.emitter_.emit('change', changeEvt);
// fire only for user-driven updates
        if (ev.internal === true) {
            this.emitter_.emit('internal-change', changeEvt);
        }
    }
...
class Rack {
    ...
    onChildValueChange_(ev) {
        ...
        this.emitter.emit('valuechange', {
            bladeController: bc,
            options: ev.options,
            internal: ev.internal,   // <-- forward it
            sender: this,
        });
    }
    onRackLayout_(_) {
...
class FolderApi extends ContainerBladeApi {
    constructor(controller, pool) {
        ...
        this.rackApi_.on('internal-change', (ev) => {
            this.emitter_.emit('internal-change', ev);
        });
    }
    get expanded() {
